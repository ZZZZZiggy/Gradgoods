...
const Buy = () => {
  ...
  const sampleRequests = {
    pending: allRequests.filter((item) => item.acceptStatus === null),
    negotiating: allRequests.filter((item) => item.acceptStatus === false),
    deal: allRequests.filter((item) => item.acceptStatus === true),
  };
    ...
    switch (tab) {
      case "pending":
        return sentRequests.has(item.id) ? (
          ...
          <button className="send-request-button" onClick={() => handleShowRequestModal(item)}>
            Send Request
          </button>
        );
